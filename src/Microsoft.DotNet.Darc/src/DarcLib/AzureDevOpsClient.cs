...
namespace Microsoft.DotNet.DarcLib
{
    public class AzureDevOpsClient : IGitRepo
    {
        ...
        public async Task<string> GetFileContentsAsync(string filePath, string repoUri, string branchOrCommit)
        {
            ...
            (string accountName, string projectName, string repositoryName) = ParseRepositoryUri(repoUri);

            HttpResponseMessage response = await ExecuteRemoteGitCommand(
                HttpMethod.Get,
                accountName,
                projectName,
                $"git/repositories/{repositoryName}/items?path={filePath}&version={branchOrCommit}&includeContent=true",
                _logger);
        ...
        {
            (string accountName, string projectName, string repoName) = ParseRepositoryUri(repoUri);
            string body;
            ...
            response = await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/refs/heads/{newBranch}",
                _logger);
            JObject responseContent = JObject.Parse(await response.Content.ReadAsStringAsync());
            ...
            await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/refs",
                _logger,
                body);
        }
            ...
            string commitMessage)
        {
            ...
            await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/pushes",
                _logger,
                body,
                "5.0-preview.2");
            ...
            string author = null)
        {
            ...
            HttpResponseMessage response = await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/pullrequests?{query}",
                _logger);
        ...
        {
            (string accountName, string projectName, int id) = ParsePullRequestUri(pullRequestUrl);
        ...
        public async Task<string> GetPullRequestRepo(string pullRequestUrl)
        {
            ...
            HttpResponseMessage response = await ExecuteRemoteGitCommand(HttpMethod.Get, null, null, url, _logger);
        ...
        {
            VssConnection connection = CreateVssConnection(pullRequestUrl);
            ...
            await client.UpdatePullRequestAsync(
                new GitPullRequest
                {
                    ...
                    {
                        SquashMerge = parameters.SquashMerge, DeleteSourceBranch = parameters.DeleteSourceBranch
                    },
        ...
        public async Task<string> CreatePullRequestCommentAsync(string pullRequestUrl, string message)
        {
            ...
            GitPullRequestCommentThread thread = await client.CreateThreadAsync(
                ...
                {
                    Comments = new List<Comment> {new Comment {CommentType = CommentType.Text, Content = message}}
                },
                team,
                repo,
                id);
        ...
        public async Task UpdatePullRequestCommentAsync(string pullRequestUrl, string commentId, string message)
        {
            ...
            await client.UpdateCommentAsync(
                new Comment {CommentType = CommentType.Text, Content = message},
                team,
                repo,
                id,
                threadId,
        ...
        public async Task<string> GetLastCommitShaAsync(string repoUri, string branch)
        {
            ...
            HttpResponseMessage response = await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/commits?branch={branch}",
                _logger);
        ...
        public async Task<string> GetPullRequestSourceBranch(string pullRequestUrl)
        {
            ...
            JObject content = JObject.Parse(await response.Content.ReadAsStringAsync());
            string sourceBranch = content["sourceRefName"].ToString();
        ...
        public async Task<bool> FileExistsAsync(string repoUri, string filePath, string branch)
        {
            ...
            try
            {
                response = await ExecuteRemoteGitCommand(
                    ...
                    accountName,
                    projectName,
                    $"git/repositories/{repoName}/items?path={filePath}&versionDescriptor[version]={branch}",
                    _logger);
            }
            ...
            string versionOverride = null)
        {
            ...
            var baseUri = "https://dev.azure.com/{accountName}/{projectName}/_apis/";
        ...
        /// <remarks>
        ///     Should we be doing this? Typically the number of HTTP clients is supposed to be kept to a minimum,
        ...
        {
            var client = new HttpClient {BaseAddress = new Uri(apiUri)};
        ...
        {
            var collectionUri = new UriBuilder(uri) {Path = "", Query = "", Fragment = ""};
        ...
        public async Task CommentOnPullRequestAsync(string pullRequestUrl, string message)
        {
            ...
            (string accountName, string projectName, string repoName) = ParseRepositoryUri(pullRequestUrl);
        ...
        public async Task GetCommitMapForPathAsync(string repoUri, string commit, List<GitFile> files, string path)
        {
            ...
            HttpResponseMessage response = await ExecuteRemoteGitCommand(
                ...
                accountName,
                projectName,
                $"git/repositories/{repoName}/items?scopePath={path}&version={commit}&includeContent=true&versionType=commit&recursionLevel=full",
                _logger);
        ...
        {
            string requestUri;
            (string accountName, string projectName, string repoName) = ParseRepositoryUri(uri);
        ...
        private static readonly string projectRepoCaptureSuffix =
            $"(?<{projectNameCaptureName}>.+)\\/_git\\/(?<{repoNameCaptureName}>.+)";

        private static readonly string form1 =
            $"(^https:\\/\\/)?dev\\.azure\\.com\\/(?<{accountNameCaptureName}>.+?)\\/{projectRepoCaptureSuffix}";

        private static readonly string form2 =
            $"(^https:\\/\\/)?(?<{accountNameCaptureName}>.+?)@dev\\.azure\\.com\\/{projectRepoCaptureSuffix}";
        ...
        private (string accountName, string projectName, string repositoryName) ParseRepositoryUri(string repoUri)
        {
            ...
            foreach (Regex regex in uriRegex)
            {
                ...
                if (m.Success)
                {
                    if (!m.Groups[accountNameCaptureName].Success || !m.Groups[projectNameCaptureName].Success ||
